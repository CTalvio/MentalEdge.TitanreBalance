...
global const float FLAME_WALL_THERMITE_DURATION = 3.8
global const float SE_FLAME_WALL_THERMITE_DURATION = 5.5
global const float PAS_SCORCH_FIREWALL_DURATION = 5.0
...
var function OnWeaponPrimaryAttack_FlameWall( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	// TITANREBALANCE change fire duration depending on scorched earth kit
...
bool function CreateThermiteWallSegment( entity projectile, int projectileCount, entity inflictor, entity movingGeo, vector pos, vector angles, int waveCount )
{
	...
	if ( projectile.proj.savedOrigin != < -999999.0, -999999.0, -999999.0 > )
	{
		...
		else
		{
			...
			// TITANREBALANCE change fire duration depending on scorched earth kit
			duration = FLAME_WALL_THERMITE_DURATION
			if( mods.contains( "pas_se_duration" ) )
				duration = SE_FLAME_WALL_THERMITE_DURATION
		}
