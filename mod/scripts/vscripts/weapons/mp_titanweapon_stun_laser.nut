...
void function StunLaser_DamagedTarget( entity target, var damageInfo )
{
	...
	if ( attacker.GetTeam() == target.GetTeam() )
	{
		...
		if ( target.IsTitan() && IsValid( attackerSoul ) && hasEnergyTransfer )
		{
			...
			{
				if ( SoulHasPassive( soulattacker, ePassives.PAS_VANGUARD_SHIELD ) )
					shieldRestoreAmount = int( 2.0 * shieldRestoreAmount )
				soulattacker.SetShieldHealth( min( soulattacker.GetShieldHealth() + shieldRestoreAmount, soulattacker.GetShieldHealthMax() ) )
			}
			if ( attacker.IsPlayer() )
