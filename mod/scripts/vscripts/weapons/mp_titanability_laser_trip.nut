...
const float LASER_TRIP_INNER_RADIUS = 250.0
...
const float PAS_LASER_TRIP_DAMAGE_HEAVY_ARMOR = 800.0
...
const float LASER_TRIP_DEPLOY_POWER = 1000.0
...
var function OnWeaponPrimaryAttack_titanweapon_laser_trip( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	entity inflictor = CreatePylonDamageInflictorHelper()
#endif
	...
	function() : ( projectile, inflictor, tower, pylon, noSpawnIdx, team, pylonOrigin, mods )
		{
			...
			if ( IsValid( soul ) )
			{
				...
				float pilotDamage = LASER_TRIP_DAMAGE
				float titanDamage = LASER_TRIP_DAMAGE_HEAVY_ARMOR
				float innerRadius = LASER_TRIP_INNER_RADIUS
				float outerRadius = LASER_TRIP_OUTER_RADIUS
				int flags = SF_ENVEXPLOSION_NO_DAMAGEOWNER

				if ( mods.contains( "pas_ion_tripwire" ) )
					titanDamage = PAS_LASER_TRIP_DAMAGE_HEAVY_ARMOR
