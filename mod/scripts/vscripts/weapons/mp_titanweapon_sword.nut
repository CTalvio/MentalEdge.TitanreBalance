...
const float PAS_RONIN_COOLDOWN = 0.2 // .15
...
void function Sword_DamagedTarget( entity target, var damageInfo )
{
	...
	// TITANREBALANCE provide highlander cooldown bonus
    if ( IsValid( soul ) && SoulHasPassive( soul, ePassives.PAS_RONIN_SWORDCORE ) )
    {
        ...
        foreach ( index in [ OFFHAND_LEFT, OFFHAND_ANTIRODEO, OFFHAND_RIGHT ] )
        {
            ...
            int newAmmo
            if ( SoulHasPassive( soul, ePassives.PAS_SHIFT_CORE ) )
				newAmmo = minint( maxAmmo, offhand.GetWeaponPrimaryClipCount() + int( maxAmmo * PAS_RONIN_COOLDOWN*2 ) )
			else
				newAmmo = minint( maxAmmo, offhand.GetWeaponPrimaryClipCount() + int( maxAmmo * PAS_RONIN_COOLDOWN ) )
