...
const float SLOW_TRAP_LIFETIME = 10.0
const float SLOW_TRAP_BUILD_TIME = 0.5 // TITANREBALANCE was 1.0
...
function DeploySlowTrap( entity projectile )
{
	...
	string noSpawnIdx = CreateNoSpawnArea( TEAM_INVALID, team, origin, SLOW_TRAP_BUILD_TIME + SLOW_TRAP_LIFETIME, range )
	SetTeam( tower, team )
...
function IgniteTrap( entity damageArea, var damageInfo, bool isExplosiveBarrel = false )
{
	...
	// TITANREBALANCE change trap fire duration depending on scorched earth kit
	float duration = FLAME_WALL_THERMITE_DURATION
	if( weapon.HasMod( "pas_se_duration" ) )
		duration = SE_FLAME_WALL_THERMITE_DURATION
	...
	float dangerousAreaRadius = range + 50
