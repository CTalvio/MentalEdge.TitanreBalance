...
void function StartShield( entity weapon )
{
	...
	print( "[DEBUG] swordblock stop regen" )
#endif
}
...
void function EndShield( entity weapon )
{
	...
	print( "[DEBUG] swordblock resume regen" )
...
// TITANREBALANCE 0.3 > 0.2 / 0.15 > 0.2
const float TITAN_BLOCK_DAMAGE_REDUCTION = 0.2
const float SWORD_CORE_BLOCK_DAMAGE_REDUCTION = 0.2
...
float function HandleBlockingAndCalcDamageScaleForHit( entity blockingEnt, var damageInfo )
{
	...
	if ( blockingEnt.IsTitan() )
	{
		...
		float exponent = pow( ((maxAmmo - currentAmmo.tofloat()) / maxAmmo), 1.75)
		int newAmmo = currentAmmo - ( ((1-exponent+TITAN_BLOCK_DAMAGE_REDUCTION)*damage)/10 ).tointeger()
